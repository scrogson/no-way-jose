...
pub static OK: Lazy<String> = Lazy::new(|| String::from("Ok"));

pub static ERROR: Lazy<String> = Lazy::new(|| String::from("Err"));
...
pub fn validate_struct<'a>(term: &Term<'a>, name: Option<&str>) -> Result<Term<'a>, Error> {
    ...
    match name {
        Some(name) => {
            let name_term = str_to_term(&term.get_env(), name).or(Err(Error::InvalidStructName))?;
