use crate::serde::{error::Error, utils};
use rustler::types::atom as atoms;
use rustler::{
...
macro_rules! try_parse_number {
    ($term:expr, $type:ty, $visitor:expr, $visit_fn:ident) => {
        if let Ok(num) = utils::parse_number(&$term) as Result<$type, Error> {
...
impl<'de, 'a: 'de> de::Deserializer<'de> for Deserializer<'a> {
        ...
        V: Visitor<'de>,
    {
        match self.term.get_type() {
            TermType::Atom => {
                if utils::is_nil(&self.term) {
                    self.deserialize_unit(visitor)
                } else if let Ok(b) = utils::parse_bool(&self.term) {
                    visitor.visit_bool(b)
                } else {
                    // unit variant (atom)
                    let string = utils::term_to_string(&self.term)?;
            ...
            TermType::Map => {
                ...
                let de = match utils::validate_struct(&self.term, None) {
            ...
            TermType::Tuple => {
                let tuple = utils::validate_tuple(self.term, None)?;
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_bool(utils::parse_bool(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i8(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i16(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i32(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i64(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_u8(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_u16(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_u32(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_u64(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_f32(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_f64(utils::parse_number(&self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_borrowed_str(utils::parse_str(self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_borrowed_bytes(utils::parse_binary(self.term)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        let tuple = utils::validate_tuple(self.term, Some(2))?;
        let name_term =
            utils::str_to_term(&self.term.get_env(), name).or(Err(Error::ExpectedStructName))?;
    ...
    where
        V: Visitor<'de>,
    {
        let tuple = utils::validate_tuple(self.term, Some(len))?;
        visitor.visit_seq(SequenceDeserializer::new(tuple.into_iter()))
    }
    ...
    where
        V: Visitor<'de>,
    {
        let mut tuple = utils::validate_tuple(self.term, Some(len + 1))?;
        let name_term =
            utils::str_to_term(&self.term.get_env(), name).or(Err(Error::ExpectedStructName))?;
    ...
    where
        V: Visitor<'de>,
    {
        let struct_name_term = utils::validate_struct(&self.term, Some(name))?;
        ...
        V: Visitor<'de>,
    {
        ...
        let variant: Option<(EnumType, Term<'a>)> = match self.term.get_type() {
            ...
            // struct variant
            TermType::Map => {
                let struct_name_term = utils::validate_struct(&self.term, None)?;
        ...
        match self.variant_type {
            EnumDeserializerType::Any | EnumDeserializerType::Newtype => {
                if let Some(term) = self.term {
                    let tuple = utils::validate_tuple(term, Some(2))?;
        ...
        V: Visitor<'de>,
    {
        match self.variant_type {
            EnumDeserializerType::Any | EnumDeserializerType::Tuple => {
                if let Some(term) = self.term {
                    let mut tuple = utils::validate_tuple(term, Some(len + 1))?;
                    let iter = tuple.split_off(1).into_iter();
                    visitor.visit_seq(SequenceDeserializer::new(iter))
                } else {
        ...
        V: Visitor<'de>,
    {
        match self.variant_type {
            EnumDeserializerType::Struct => {
                if let Some(term) = self.term {
                    utils::validate_struct(&term, Some(&self.variant))?;
...
impl<'de, 'a: 'de> de::Deserializer<'de> for VariantNameDeserializer<'a> {
        ...
        V: Visitor<'de>,
    {
        match self.variant.get_type() {
            TermType::Atom => {
                let string =
                    utils::term_to_string(&self.variant).or(Err(Error::InvalidVariantName))?;
                visitor.visit_string(string)
            }
            TermType::Binary => visitor.visit_string(utils::term_to_str(&self.variant)?),
            TermType::Integer => visitor.visit_string(utils::term_to_str(&self.variant)?),
            TermType::Float => visitor.visit_string(utils::term_to_str(&self.variant)?),
