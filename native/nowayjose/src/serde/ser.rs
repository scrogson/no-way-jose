...
impl<'a> ser::Serializer for Serializer<'a> {
    ...
    fn serialize_unit_variant(
        ...
        utils::str_to_term(&self.env, variant).or(Err(Error::InvalidVariantName))
    }
    ...
    where
        T: ?Sized + ser::Serialize,
    {
        let name_term = utils::str_to_term(&self.env, name).or(Err(Error::InvalidVariantName))?;
    ...
    fn serialize_struct(
        ...
        let name_term = utils::str_to_term(&self.env, name).or(Err(Error::InvalidStructName))?;
...
impl<'a> ser::SerializeStruct for MapSerializer<'a> {
    ...
    where
        T: ?Sized + Serialize,
    {
        let key_term = utils::str_to_term(&self.ser.env, key).or(Err(Error::InvalidStructKey))?;
